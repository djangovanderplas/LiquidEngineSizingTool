...
class Engine:
    ...
    def initial_size_geometry(self, nozzle_type='bell'):
        ...
        self.contraction_ratio = 8.0 * self.throat_diameter ** -0.6 + 1.25 # Emperically determined, mildly sus
        self.chamber_area = self.throat_area * self.contraction_ratio
        ...
        print(self.chamber_diameter)

        # Size nozzle
        match nozzle_type:
            case 'bell':
                self.nozzle = NozzleSizer.BellNozzle(self.expansion_ratio, self.contraction_ratio, self.throat_diameter / 2, length_percentage=0.8, nozzle_arc_scalar=1.5, entrance_angle=135)
            case 'conical':
                self.nozzle = NozzleSizer.ConicalNozzle(self.expansion_ratio, self.throat_diameter / 2, length_percentage=1, conical_half_angle=15, nozzle_arc_scalar=1.5, entrance_angle=135)

    def plot_geometry(self, ax1):
        ax1.set_aspect('equal')
        self.nozzle.plot2D(ax1)

        x_vals = np.linspace(-self.chamber_length, 0, 100)
        y_vals = np.ones(len(x_vals)) * self.chamber_diameter/2

        ax1.plot(x_vals, y_vals, 'k')
